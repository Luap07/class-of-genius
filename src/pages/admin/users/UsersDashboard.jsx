...
* IMPORTANT:
* Student enrollment records can come back with different
* property names depending on the backend query/alias.
*
* Always resolve the real enrollment identifier from all
* supported possibilities before sending it to the API.
  */
  const getStudentEnrollmentId = (student) => {
  if (!student) {
  return "";
  }
...
try {
  ...
  if (type === "student") {
    const enrollmentId =
      getStudentEnrollmentId(user);

    /*
     * This is the critical fix.
     * Do not send an empty enrollmentId.
     */
    if (!enrollmentId) {
      console.error(
        "Student record received by UsersDashboard:",
        user
      );

      throw new Error(
        "This student record does not contain a valid enrollment ID."
      );
    }

    /*
     * Send every common identifier name.
     * The backend can therefore resolve the student
     * even if its route is currently reading a
     * different property name.
     */
    payload = {
      enrollmentId,
      enrollment_id: enrollmentId,
      enrollment_reference: enrollmentId,
    ...
    const reference =
      getTutorReference(user);

    if (!reference) {
      console.error(
        "Tutor record received by UsersDashboard:",
        user
      );

      throw new Error(
        "This tutor record does not contain a valid tutor reference."
      );
    }
