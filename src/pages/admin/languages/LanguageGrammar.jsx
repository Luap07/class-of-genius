...
import { Loader2, Search, Plus, BookOpen, Star, CheckCircle } from "lucide-react";
import { supabase } from "../../../lib/supabaseClient";
import GrammarReaderBuilder from "../../../components/admin/languages/GrammarReaderBuilder";


export default function LanguageGrammar({ language, refresh }) {
  ...
  const stats = useMemo(() => {
    return {
      ...
      featured: grammar.filter((item) => item.featured).length,
      rules: grammar.filter((item) => item.rules).length,
      exercises: grammar.filter((item) => item.exercises).length,
    };
  }, [grammar]);
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-slate-400">
              Manage grammar rules, explanations, examples and practice activities.
            </p>
          </div>
          <button
            onClick={() => {
      ...
      <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-400">Rules Added</p>
              <h3 className="mt-2 text-4xl font-black text-white">{stats.rules}</h3>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="mt-2 text-4xl font-black text-white">{stats.exercises}</h3>
            </div>
      ...
      {filteredGrammar.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.03] py-24 text-center">
          <BookOpen className="mx-auto mb-6 h-16 w-16 text-slate-600" />
          <h3 className="text-2xl font-black text-white">No Grammar Topics</h3>
          <p className="mt-3 text-slate-400">Add your first grammar topic.</p>
          <button
            onClick={() => {
              setEditingGrammar(null);
              setShowModal(true);
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-indigo-300">{item.topic || "Grammar"}</p>
                </div>
                {item.featured && (
              ...
              <div className="mt-6 space-y-5">
                {item.explanation && (
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">Explanation</p>
                    <p className="line-clamp-4 text-sm leading-7 text-slate-300">{item.explanation}</p>
                  </div>
                )}
                {item.rules && (
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">Rules</p>
                    <p className="line-clamp-4 text-sm text-slate-400">{item.rules}</p>
                  </div>
                )}
                {item.examples && (
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">Examples</p>
                    <p className="line-clamp-4 text-sm text-slate-400">{item.examples}</p>
                  </div>
                )}
                {item.exercises && (
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">Exercises</p>
                    <p className="line-clamp-3 text-sm text-slate-400">{item.exercises}</p>
                  </div>
                )}
              </div>

              <div className="mt-8 flex items-center justify-between border-t border-white/10 pt-5">
                <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Grammar Topic</span>
...
function GrammarModal({ grammar, language, close, refresh }) {
  const [saving, setSaving] = useState(false);
  const [title, setTitle] = useState(grammar?.title || "");
  const [topic, setTopic] = useState(grammar?.topic || "");
  const [level, setLevel] = useState(grammar?.level || "Beginner");
  const [explanation, setExplanation] = useState(grammar?.explanation || "");
  const [rules, setRules] = useState(grammar?.rules || "");
  const [examples, setExamples] = useState(grammar?.examples || "");
  const [exercises, setExercises] = useState(grammar?.exercises || "");
  const [notes, setNotes] = useState(grammar?.notes || "");
  const [featured, setFeatured] = useState(grammar?.featured || false);
  const [sortOrder, setSortOrder] = useState(grammar?.sort_order || 0);
  ...
  const saveGrammar = async () => {
    try {
      ...
      alert("Failed to save grammar topic.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-3xl border border-white/10 bg-[#08111f] p-8">
        <div className="mb-8 flex items-center justify-between">
          <h2 className="text-3xl font-black text-white">
            {grammar ? "Edit Grammar Topic" : "Create Grammar Topic"}
          </h2>
          <button onClick={close} className="rounded-xl bg-white/10 px-4 py-2 hover:bg-white/25 text-white">
            Close
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <select
              ...
              className="rounded-2xl border border-white/10 bg-black/20 p-4 text-white outline-none focus:border-indigo-500"
            >
              <option className="bg-slate-900 text-white">Beginner</option>
              <option className="bg-slate-900 text-white">Intermediate</option>
              <option className="bg-slate-900 text-white">Advanced</option>
            </select>
            <input
              type="number"
              value={sortOrder}
          ...
          <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 p-4 text-white cursor-pointer">
            <input
              type="checkbox"
              checked={featured}
              onChange={(e) => setFeatured(e.target.checked)}
              className="h-5 w-5 rounded border-white/10 bg-black/40 text-indigo-500 focus:ring-0"
            />
          ...
          <div className="flex justify-end gap-4 pt-5">
            <button
              onClick={close}
              className="rounded-2xl border border-white/10 px-7 py-3 font-semibold text-white hover:bg-white/10"
            >
              Cancel
            </button>
            <button
