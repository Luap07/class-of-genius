...
const handleOpenContent = (language) => {
...
const closeContentManager = () => {
  ...
  =============================== */
  const [materialLanguage, setMaterialLanguage] = useState(null);
  ...
  const handleCreateLanguage = async () => {
    try {
      ...
      console.error("Create Language:", error);
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (selectedLanguage?.id === deletingLanguage.id) {
      ...
      console.error("Delete Language:", error);
    } finally {
      setDeleting(false);
    }
  };
  ...
  const regions = useMemo(() => {
    return [
      "all",
      ...new Set(languages.map((item) => item.region).filter(Boolean)),
    ];
  }, [languages]);

  const totalLanguages = languages.length;
  const activeLanguages = languages.filter((item) => item.active).length;
  const inactiveLanguages = languages.filter((item) => !item.active).length;
  const totalRegions = new Set(
    languages.map((item) => item.region).filter(Boolean)
  ).size;
      ...
      <div className="min-h-screen bg-[#030712] p-6 text-white">
        ...
        onClick={() => navigate("/admin/languages/content")}
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-2 rounded-2xl bg-white/10 px-5 py-3 font-bold hover:bg-white/20 disabled:opacity-50 cursor-pointer"
            >
              <RefreshCw
                size={18}
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 font-bold hover:bg-indigo-500 cursor-pointer"
            >
              <Plus size={20} />
              Add Language
            </button>
