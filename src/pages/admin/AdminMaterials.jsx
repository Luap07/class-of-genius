...
const STORAGE_BUCKET = "academy-materials";
...
const getMaterialFileUrl = (material) => {
...
const AdminMaterials = () => {
  ...
  const handleCreateMaterial = async (event) => {
    ...
    if (!form.title.trim()) {
      setMessage({
        type: "error",
        text: "Please enter the material title.",
      });
    ...
    try {
      ...
      const safeFileName = form.file.name
        .replace(/[^a-zA-Z0-9._-]/g, "_");

      const filePath = `textbooks/${Date.now()}-${safeFileName}`;
      ...
      if (!fileUrl) {
        throw new Error(
          "Unable to generate the material file URL."
        );
      }
  ...
  const handleDeleteMaterial = async (
    ...
    const confirmed = window.confirm(
      `Delete "${material.title}"? This will remove the material from the library.`
    );
        ...
        {loading ? (
          ...
          <div className="flex min-h-[400px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 bg-slate-900/40 px-6 text-center">
            ...
            <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
              Your Academy textbook library is
              currently empty. Add a textbook or
